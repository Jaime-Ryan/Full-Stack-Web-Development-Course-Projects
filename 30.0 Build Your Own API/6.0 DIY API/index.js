  ...
  if (type) {
    ...
    if (filteredJokes.length > 0) {
      ...
      res.status(404).json({ 
        ...
        hint: `Available types: ${[...new Set(jokes.map(joke => joke.jokeType))].join(", ")}`
      });
    }
  } else {
...
app.post("/jokes", (req, res) => {
  ...
  if (!jokeText || !jokeType) {
    return res.status(400).json({ 
      error: "Both jokeText and jokeType are required." 
    });
  }
...
app.put("/jokes/:id", (req, res) => {
  ...
  if (!jokeText || !jokeType) {
    return res.status(400).json({ 
      error: "Both jokeText and jokeType are required for PUT." 
    });
  }
...
app.delete("/all", (req, res) => {
  ...
  const deletedCount = jokes.length;
